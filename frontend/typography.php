...
$style = $menu_style = $submenu_style = $topbar_style = "";
...
if (trim($submenuType) == 'custom') {
   ...
   $submenu_style = '.nav-submenu-container .nav-submenu > li, .jddrop-content .megamenu-item .megamenu-menu li, .nav-submenu, .astroid-sidebar-menu .nav-item-submenu {';
...
if (trim($topbarType) == 'custom') {
	$topbar_font = $template->params->get('top_bar_typography_options');
	$topbar_fontface = str_replace('+', ' ', explode(":", $topbar_font->font_face));
	$topbar_style = '.top-bar {';
	if (isset($topbar_fontface[0]) && !empty($topbar_fontface[0])) {

		if (isset($libraryFonts[$topbar_fontface[0]])) {
			$topbar_style .= 'font-family: ' . $libraryFonts[$topbar_fontface[0]]['name'] . ';';
			AstroidFrameworkHelper::loadLibraryFont($libraryFonts[$topbar_fontface[0]], $template);
		} else {
			$topbar_style .= 'font-family: ' . $topbar_fontface[0] . ';';
			if (!AstroidFrameworkHelper::isSystemFont($topbar_fontface[0])) {
				array_push($ast_fontfamily, $topbar_font->font_face);
			}
		}
	}
	if (isset($topbar_font->font_size) && !empty($topbar_font->font_size)) {
		$font_size_unit = isset($topbar_font->font_size_unit) ? $topbar_font->font_size_unit : 'em';
		$topbar_style .= 'font-size: ' . $topbar_font->font_size . $font_size_unit . ';';
	}
	if (isset($topbar_font->font_color) && !empty($topbar_font->font_color)) {
		$topbar_style .= 'color: ' . $topbar_font->font_color . ';';
	}
	if (isset($topbar_font->letter_spacing) && !empty($topbar_font->letter_spacing)) {
		$letter_spacing_unit = isset($topbar_font->letter_spacing_unit) ? $topbar_font->letter_spacing_unit : 'em';
		$topbar_style .= 'letter-spacing: ' . $topbar_font->letter_spacing . $letter_spacing_unit . ';';
	}
	if (isset($topbar_font->font_weight) && !empty($topbar_font->font_weight)) {
		$topbar_style .= 'font-weight: ' . $topbar_font->font_weight . ';';
	}
	if (isset($topbar_font->line_height) && !empty($topbar_font->line_height)) {
		$line_height_unit = isset($topbar_font->line_height_unit) ? $topbar_font->line_height_unit : 'em';
		$topbar_style .= 'line-height: ' . $topbar_font->line_height . $line_height_unit . ';';
	}
	if (isset($topbar_font->text_transform) && !empty($topbar_font->text_transform)) {
		$topbar_style .= 'text-transform: ' . $topbar_font->text_transform . ';';
	}
	$topbar_style .= '}';
}
...
if ($in_head) {
   ...
   $template->addStyleDeclaration($topbar_style);
} else {
   if (!empty($ast_fontfamily_list)) {
